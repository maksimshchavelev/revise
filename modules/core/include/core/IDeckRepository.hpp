...
class IDeckRepository {
  public:
    ...
    virtual std::expected<QVector<core::DeckSummary>, QString> get_deck_summaries(const QDateTime& now) = 0;
    virtual std::expected<QVector<core::Card>, QString>        get_cards(int deckId) = 0;
    virtual std::expected<core::Deck, QString>                 get_deck(int deckId) = 0;

    virtual std::expected<int, QString>  create_deck(const core::Deck& deck) = 0;
    virtual std::expected<void, QString> update_deck(const core::Deck& deck) = 0;
    ...
    virtual std::expected<void, QString>       update_card(const core::Card& card) = 0;
    virtual std::expected<void, QString>       insert_cards_batch(int deckId, const QVector<core::Card>& cards) = 0;
    virtual std::expected<void, QString>       update_card_backs_batch(const QVector<CardBackUpdate>& cards) = 0;
    virtual std::expected<void, QString>       insert_card(int deckId, const core::Card& card) = 0;
    virtual std::expected<bool, QString>       is_card_exists(int deckId, const QString& card_front) = 0;
    virtual std::expected<core::Card, QString> get_card(int cardId) = 0;
