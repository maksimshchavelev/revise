...
SqlEventStorage::SqlEventStorage(Database& db) : m_db(db) {
...
std::expected<void, QString> SqlEventStorage::migrate_v1_to_v2() {
    ...
    if (!q.exec("CREATE INDEX idx_events_created_at ON events(created_at)")) {
        return std::unexpected(QString("Failed to create indexes for field 'created_at' for table 'events'. Query: %1, cause: %2")
                                   .arg(q.lastQuery())
                                   .arg(q.lastError().text()));
    }

    if (!q.exec("CREATE INDEX idx_events_sync_state ON events(sync_state)")) {
        return std::unexpected(QString("Failed to create indexes for field 'sync_state' for table 'events'. Query: %1, cause: %2")
                                   .arg(q.lastQuery())
                                   .arg(q.lastError().text()));
    }
